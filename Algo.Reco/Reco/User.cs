...
namespace Algo
{
    public partial class User
    {
        ...
        IEnumerable<SimilarUser> _friends;

        public IEnumerable<SimilarUser> GetSimilarUser(RecoContext ctx)
        {
            if( _friends == null )
            {
                _friends = ctx.Users.Take(500).Where( u => u != this )
                    .Select( u => new SimilarUser
                    {
                        Friend = u,
                        Similarity = ctx.SimilarityPearson( this, u )
                    } )
